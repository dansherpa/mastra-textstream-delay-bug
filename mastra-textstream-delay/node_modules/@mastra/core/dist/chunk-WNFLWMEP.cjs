...
function getPackageRoot() {
  try {
    const require2 = module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('chunk-WNFLWMEP.cjs', document.baseURI).href)) || "file://");
...
var GatewayRegistry = class _GatewayRegistry {
  ...
  async syncGateways(forceRefresh = false, writeToSrc = false) {
    ...
    try {
      const { ModelsDevGateway: ModelsDevGateway2 } = await import('./models-dev-HZ3LZHPX.cjs');
