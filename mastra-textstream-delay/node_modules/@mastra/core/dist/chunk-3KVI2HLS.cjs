...
var MastraScorer = class _MastraScorer {
  ...
  toMastraWorkflow() {
    const workflowSteps = this.steps.map((scorerStep) => {
      return chunkWUFFST2N_cjs.createStep({
        id: scorerStep.name,
    ...
    const workflow = chunkWUFFST2N_cjs.createWorkflow({
  ...
  async executePromptStep(scorerStep, tracingContext, context) {
    ...
    const resolvedModel = await chunkWNFLWMEP_cjs.resolveModelConfig(modelConfig);
    const judge = new chunkWUFFST2N_cjs.Agent({
      name: "judge",
      model: resolvedModel,
      instructions,
    ...
    if (scorerStep.name === "generateScore") {
      ...
      if (resolvedModel.specificationVersion === "v2") {
        result = await chunkWUFFST2N_cjs.tryGenerateWithJsonFallback(judge, prompt, {
          structuredOutput: {
            schema
          },
          tracingContext
...
function isWorkflow(target) {
  return target instanceof chunkWUFFST2N_cjs.Workflow;
}
...
//# sourceMappingURL=chunk-3KVI2HLS.cjs.map
//# sourceMappingURL=chunk-3KVI2HLS.cjs.map