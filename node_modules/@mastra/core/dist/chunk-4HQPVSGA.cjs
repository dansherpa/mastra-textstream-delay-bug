...
async function validateAndSaveScore(storage, payload) {
  const payloadToSave = chunkUD7DS7OY_cjs.saveScorePayloadSchema.parse(payload);
...
exports.Mastra = class Mastra {
  ...
  constructor(config) {
    ...
    const workflowEventProcessor = new chunkDOLSYVNJ_cjs.WorkflowEventProcessor({
      mastra: this
    });
    ...
    if (config?.observability) {
      chunkLZFCR2SE_cjs.setupAITracing(config.observability);
    }
...
`
  ...
  registerAITracingExporters() {
    const allTracingInstances = chunkLZFCR2SE_cjs.getAllAITracing();
   ...
   */
  async shutdown() {
    await chunkLZFCR2SE_cjs.shutdownAITracingRegistry();
...
//# sourceMappingURL=chunk-4HQPVSGA.cjs.map
//# sourceMappingURL=chunk-4HQPVSGA.cjs.map